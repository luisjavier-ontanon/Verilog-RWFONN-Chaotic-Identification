...
module adder4_sec #(parameter nbits = 32,fraccion = 20)	
	( 
	input clk, rst,
	input  [nbits-1:0] A, B, C, D, 
	...
	always @(posedge clk or negedge rst) begin
		if (!rst) begin
			...
			sum <= $signed(A) + $signed(B) + $signed(C)+ $signed(D);	  // por default las restas son en C2  < es sintaxis no-bloqueante
		end
	end
endmodule